...
2. Create and approve `docs/product/constitution.md` — governance principles (use the immutable + supersede evolution strategy)
3. Run Phase 0.5 Product Vision before the first PRD
4. Create approved, versioned `docs/product/vision.md` through the Vision Grill (use the immutable + supersede evolution strategy)
5. Proceed to the first PRD only after the Foundation Gate passes

When running the Vision Grill, prefer `grill-with-docs` if it is installed. If it is missing, notify the user up front, recommend installing it for doc-aware interrogation, and continue with the built-in fallback only if the user declines installation or asks to proceed.
...
Phase 1 brainstorming and design exploration may happen before the Foundation Gate, but the first Phase 2 PRD cannot start until `docs/product/constitution.md` is approved and approved, versioned `docs/product/vision.md` exists.
